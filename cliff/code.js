...
function draw(seconds) {
    ...
    const green = new Float32Array([0.2, 0.6,0.1,1.0])
    const red = new Float32Array([0.6, 0.3,0.3,1.0])
    gl.uniform4fv(program.uniforms.g, green)
    gl.uniform4fv(program.uniforms.r, red)

    let rot_mat = matrix.m4rotX(-Math.PI/2)
    //let rot_mat = matrix.m4rotX(0)
    let ma = matrix.m4mul(rot_mat, matrix.m4rotZ(seconds/1.5),
...
window.addEventListener('load', async (event) => {
    ...
    window.terrain = setupGeomery(1, -1, 30, 50)
    tick(0)
