...
function draw(seconds) {
    ...
    if (keysBeingPressed['ArrowLeft']){
     ...
     if ((keysBeingPressed['g'])){
        window.f = 0.8*window.f
     }
     if ((keysBeingPressed['h'])){
        window.f = 1.25*window.f
     }
     if ((keysBeingPressed['f'])){
        if (fog_available){
            window.fog_available = false
        }else{
            window.fog_available = true
        }
    ...
    let fog = new Float32Array([1,1,1])

    let buf_color = gl.createBuffer()
    gl.bindBuffer(gl.ARRAY_BUFFER, buf_color)
    gl.bufferData(gl.ARRAY_BUFFER, fog, gl.STATIC_DRAW)
    gl.uniform3fv(program.uniforms.fog, fog)
    
    let vis = 0
    if (window.fog_available){
        vis = -1.0*window.f
    }else{
        vis =-1.0
    }
    //console.log(vis)
    gl.uniform1f(program.uniforms.v, vis)
