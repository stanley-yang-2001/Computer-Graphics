...
function setupGeomery(max, min, grid_size, fault, weathering) {
    ...
    for (let i = 0; i < weathering; i++){
        for (let j = 0; j < grid.length; j++){
            let hold = []
            if (j%grid_size != 0){
                hold.push(grid[j-1][2])
            }
            if (j%grid_size != grid_size-1){
                hold.push(grid[j+1][2])
            }
            if (j >= grid_size){        // top position exist
                hold.push(grid[j-grid_size][2])
                if(j%grid_size != 0){
                    hold.push(grid[j-grid_size-1][2])
                }if(j%grid_size != grid_size-1){
                    hold.push(grid[j-grid_size+1][2])
                }
            }
            if (j <= (grid_size-1)*grid_size -1){
                hold.push(grid[j+grid_size][2])
                if(j%grid_size != 0){
                    hold.push(grid[j+grid_size-1][2])
                }if(j%grid_size != grid_size-1){
            ...
            ave_height.push(average(hold))
        }
        for (let k = 0; k < grid.length; k++){
            grid[k][2] = (grid[k][2]+ave_height[k])/2
        }
        ave_height.length = 0
    }
    // grid normalization
...
function draw(seconds) {
    ...
    //let rot_mat = matrix.m4rotX(0)
    let ma = matrix.m4mul(rot_mat, matrix.m4rotZ(seconds/3),
    matrix.m4scale(0.8,0.8,0.8), matrix.m4trans(0,0,0))

    // x- left ot right, y - depth, z = up or down
     
     
    let v = matrix.m4view([0,2,2], [0,0,0], [0,1,0])
...
window.addEventListener('load', async (event) => {
    ...
    window.terrain = setupGeomery(1, -1, 0, 0, 0)
    tick(0)
...
document.querySelector('#submit').addEventListener('click', event => {
    ...
    window.terrain = setupGeomery(1, -1, gridsize, faults, weathering)
})
